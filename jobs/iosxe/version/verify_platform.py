"""Atomic version test: platform should match baseline."""
...
MISSING_FIELD_REASON = "{device} does not report platform in '{command}' output"
MISSING_LEARNED_BASELINE = (
    "{device} is missing learned version platform baseline parameters"
)
MISSING_CURRENT_STATE = "{device} is missing current version platform state"
VALUE_MISMATCH = (
    "{device}'s version platform has drifted - we expected "
    "'{expected_value}' but found '{current_value}' instead."
)
VALUE_MATCH = (
    "{device}'s current version platform ({current_value}) matches "
    "baseline parameters ({expected_value})"
)


class VersionPlatformDeviceParameters(TypedDict):
    value: str


class VersionPlatformParameters(TypedDict):
    devices: dict[str, VersionPlatformDeviceParameters]


class VerifyVersionPlatform(LearningTestCase[VersionPlatformParameters]):
    """Value check for parsed version platform."""

    DESCRIPTION = (
        "Validate that each device's version platform remains aligned with "
        "the learned baseline."
    )
    SETUP = (
        "- Devices are reachable over SSH.\n"
        "- The `show version` command is supported on applicable targets.\n"
        "- Learned version platform baseline parameters are available for testing mode."
    )
    PROCEDURE = (
        "- Execute `show version` on each applicable device.\n"
        "- Parse the command output and extract `platform`.\n"
    ...
    PASS_FAIL_CRITERIA = (
        "- Pass when the current version platform equals the learned baseline.\n"
    ...
    async def gather_state(self, context: Context) -> VersionPlatformParameters:
        devices: dict[str, VersionPlatformDeviceParameters] = {}
        for device in context.targets:
            result = await context.broker.execute(device, self.command)
    ...
    async def compare_state(
        self,
        *,
        expected: VersionPlatformParameters,
        current: VersionPlatformParameters,
        context: Context,
    ) -> None:
        for device in context.targets:
            try:
