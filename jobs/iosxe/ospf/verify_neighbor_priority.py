"""Atomic OSPF neighbor test: priority should match baseline."""
...
MISSING_LEARNED_BASELINE = (
    "{device} is missing learned OSPF neighbor priority baseline parameters"
)
MISSING_CURRENT_STATE = "{device} is missing current OSPF neighbor priority state"
MISSING_NEIGHBOR = (
...
VALUE_MISMATCH = (
    ...
    "priority has drifted - we expected '{expected_value}' but found "
    "'{current_value}' instead."
)
VALUES_MATCH = (
    "{device}'s current OSPF neighbor priority values match baseline parameters"
)


class OspfNeighborPriorityDeviceParameters(TypedDict):
    values: dict[str, dict[str, str]]


class OspfNeighborPriorityParameters(TypedDict):
    devices: dict[str, OspfNeighborPriorityDeviceParameters]


class VerifyOspfNeighborPriority(LearningTestCase[OspfNeighborPriorityParameters]):
    """Value check for parsed OSPF neighbor priority."""

    DESCRIPTION = (
        "Validate that each OSPF neighbor's priority remains aligned with "
        "the learned baseline."
    )
    SETUP = (
        "- Devices are reachable over SSH.\n"
        "- The `show ip ospf neighbor` command is supported on applicable targets.\n"
        "- Learned OSPF neighbor priority baseline parameters are available for testing mode."
    )
    PROCEDURE = (
        "- Execute `show ip ospf neighbor` on each applicable device.\n"
        "- Parse the command output and extract `neighbors.*.*` priority.\n"
        ...
        "  - {{ key }}: expected priority={{ val }}\n"
        "{% endfor %}"
        "{% endfor %}"
    )
    PASS_FAIL_CRITERIA = (
        "- Pass when each neighbor's priority matches the learned baseline.\n"
    ...
    async def gather_state(self, context: Context) -> OspfNeighborPriorityParameters:
        devices: dict[str, OspfNeighborPriorityDeviceParameters] = {}
        for device in context.targets:
            result = await context.broker.execute(device, self.command)
    ...
    async def compare_state(
        self,
        *,
        expected: OspfNeighborPriorityParameters,
        current: OspfNeighborPriorityParameters,
        context: Context,
    ) -> None:
        for device in context.targets:
            try:
