"""Atomic OSPF neighbor test: state should match baseline."""
...
MISSING_LEARNED_BASELINE = (
    "{device} is missing learned OSPF neighbor state baseline parameters"
)
MISSING_CURRENT_STATE = "{device} is missing current OSPF neighbor state state"
MISSING_NEIGHBOR = (
...
VALUE_MISMATCH = (
    ...
    "state has drifted - we expected '{expected_value}' but found "
    "'{current_value}' instead."
)
VALUES_MATCH = "{device}'s current OSPF neighbor state values match baseline parameters"


class OspfNeighborStateDeviceParameters(TypedDict):
    values: dict[str, dict[str, str]]


class OspfNeighborStateParameters(TypedDict):
    devices: dict[str, OspfNeighborStateDeviceParameters]


class VerifyOspfNeighborState(LearningTestCase[OspfNeighborStateParameters]):
    """Value check for parsed OSPF neighbor state."""

    DESCRIPTION = (
        "Validate that each OSPF neighbor's state remains aligned with "
        "the learned baseline."
    )
    SETUP = (
        "- Devices are reachable over SSH.\n"
        "- The `show ip ospf neighbor` command is supported on applicable targets.\n"
        "- Learned OSPF neighbor state baseline parameters are available for testing mode."
    )
    PROCEDURE = (
    ...
    async def gather_state(self, context: Context) -> OspfNeighborStateParameters:
        devices: dict[str, OspfNeighborStateDeviceParameters] = {}
        for device in context.targets:
            result = await context.broker.execute(device, self.command)
    ...
    async def compare_state(
        self,
        *,
        expected: OspfNeighborStateParameters,
        current: OspfNeighborStateParameters,
        context: Context,
    ) -> None:
        for device in context.targets:
            try:
