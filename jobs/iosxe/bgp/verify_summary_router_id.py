"""Atomic BGP summary test: router ID should match baseline."""
...
MISSING_FIELD_REASON = "{device} does not report router_id in '{command}' output"
MISSING_LEARNED_BASELINE = (
    "{device} is missing learned BGP summary router ID baseline parameters"
)
MISSING_CURRENT_STATE = "{device} is missing current BGP summary router ID state"
VALUE_MISMATCH = (
    "{device}'s BGP summary router ID has drifted - we expected "
    "'{expected_value}' but found '{current_value}' instead."
)
VALUE_MATCH = (
    "{device}'s current BGP summary router ID ({current_value}) matches "
    "baseline parameters ({expected_value})"
)


class BgpSummaryRouterIdDeviceParameters(TypedDict):
    value: str


class BgpSummaryRouterIdParameters(TypedDict):
    devices: dict[str, BgpSummaryRouterIdDeviceParameters]


class VerifyBgpSummaryRouterId(LearningTestCase[BgpSummaryRouterIdParameters]):
    """Value check for parsed BGP summary router ID."""

    DESCRIPTION = (
        "Validate that each device's BGP summary router ID remains aligned with "
        "the learned baseline."
    )
    SETUP = (
        "- Devices are reachable over SSH.\n"
        "- The `show ip bgp summary` command is supported on applicable targets.\n"
        "- Learned BGP summary router ID baseline parameters are available for testing mode."
    )
    PROCEDURE = (
        "- Execute `show ip bgp summary` on each applicable device.\n"
        "- Parse the command output and extract `router_id`.\n"
    ...
    PASS_FAIL_CRITERIA = (
        "- Pass when the current BGP summary router ID equals the learned baseline.\n"
    ...
    async def gather_state(self, context: Context) -> BgpSummaryRouterIdParameters:
        devices: dict[str, BgpSummaryRouterIdDeviceParameters] = {}
        for device in context.targets:
            result = await context.broker.execute(device, self.command)
            ...
            address_families = parsed.get("address_families") or {}
            af = next(iter(address_families.values()), None) or {}
            value: Any = af.get("router_id")
    ...
    async def compare_state(
        self,
        *,
        expected: BgpSummaryRouterIdParameters,
        current: BgpSummaryRouterIdParameters,
        context: Context,
    ) -> None:
        for device in context.targets:
            try:
